...
import { useDraft } from '../state/characterDraft'; // ✅ point to your Zustand store

type WizardStep = 'race' | 'career' | 'stats' | 'name' | 'summary';

interface WizardProps {
  onComplete?: (character: any) => void; // or Draft, if you import it
  onCancel?: () => void;
}
...
export default function CharacterWizard({ onComplete, onCancel }: WizardProps) {
  ...
  // Derived from draft state
  const hasRolled = !!draft.hasRolled;
  ...
  const selectCareer = (careerId: string) => {
    setCareer(careerId);          // ensure store sets magicFloor internally…
    finalizeDerived();            // …then recompute derived once
  };

  const rollCharacteristics = () => {
    if (!draft.raceId) return;
    rollStats(draft.raceId); // ✅ store handles rolling + derived
  };
